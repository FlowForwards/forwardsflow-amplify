// BankRiskDashboard.tsx - Credit Risk Analyst Dashboard
// Place in: src/components/dashboards/BankRiskDashboard.tsx
...
import { apiService, Loan, BankMetrics } from '../../services/apiService';
...
const StatCard: React.FC<{
  ...
  trend?: number;
  alert?: boolean;
}> = ({ title, value, icon: Icon, color, trend, alert }) => {
...
const BankRiskDashboard: React.FC<BankRiskDashboardProps> = ({ user }) => {
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [activeTab, setActiveTab] = useState<'overview' | 'portfolio' | 'scoring' | 'defaults' | 'stress'>('overview');
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-rose-100 mt-1">
              {user.tenantName} • Portfolio Risk Monitoring
            </p>
          </div>
