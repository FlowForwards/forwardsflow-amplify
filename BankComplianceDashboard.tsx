// BankComplianceDashboard.tsx - Compliance Officer Dashboard
// Place in: src/components/dashboards/BankComplianceDashboard.tsx
...
import { apiService, AMLAlert } from '../../services/apiService';
...
interface BankComplianceDashboardProps {
  user: User;
}
...
const BankComplianceDashboard: React.FC<BankComplianceDashboardProps> = ({ user }) => {
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [activeTab, setActiveTab] = useState<'overview' | 'aml' | 'kyc' | 'reports' | 'audit'>('overview');
  ...
  const loadData = async (showRefresh = false) => {
    ...
    try {
      const alerts = await apiService.getAMLAlerts(user.tenantId);
      setAmlAlerts(alerts);
    } catch (error) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-amber-100 mt-1">
              {user.tenantName} • AML/KYC Monitoring
            </p>
          </div>
