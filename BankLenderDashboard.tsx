// BankLenderDashboard.tsx - Lending Officer Dashboard
// Place in: src/components/dashboards/BankLenderDashboard.tsx
...
import { apiService, LoanApplication, Loan, BankMetrics } from '../../services/apiService';
...
interface BankLenderDashboardProps {
  user: User;
}
...
}> = ({ application, onApprove, onReject, onView }) => {
...
// ============================================
const BankLenderDashboard: React.FC<BankLenderDashboardProps> = ({ user }) => {
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [activeTab, setActiveTab] = useState<'queue' | 'active' | 'collections' | 'analytics'>('queue');
  ...
  const loadData = async (showRefresh = false) => {
    ...
    try {
      const [metricsData, appsData, loansData] = await Promise.all([
        apiService.getBankMetrics(user.tenantId),
        apiService.getLoanApplications(user.tenantId),
  ...
  const handleApprove = async (applicationId: string) => {
    await apiService.updateApplicationStatus(applicationId, 'approved');
    loadData(true);
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100 mt-1">
              {user.tenantName} • {pendingApps.length} applications pending review
            </p>
          </div>
